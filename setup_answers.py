...
def build_pipeline():
    ...
    # Skip enrichment to bypass crashes
    enriched = []
    if enriched:
        all_chunks = [{"text": e.enriched_text, "metadata": e.auto_metadata} for e in enriched]
        print(f"  OK Enriched {len(enriched)} chunks ({time.time()-t0:.1f}s)", flush=True)
    else:
        print(f"  OK Using {len(all_chunks)} raw chunks (M5 not implemented or no API key)", flush=True)

    print("\n[2/3] Indexing (BM25 + Dense)...", flush=True)
    t0 = time.time()
    search = HybridSearch()
    search.index(all_chunks)
    print(f"  OK Indexed {len(all_chunks)} chunks into Qdrant & BM25 ({time.time()-t0:.1f}s)", flush=True)
