...
if __name__ == "__main__":
    print("Running swap-and-average judge on 20 pairs from answers_50q.json...")
    
    with open("answers_50q.json", encoding="utf-8") as f:
        answers_data = json.load(f)
    
    pairs = []
    for item in answers_data[:20]:
        pairs.append({
            "q": item["question"],
            "a_a": item["answer"],
            "a_b": item["ground_truth"]
        })

    results = []
    for i, pair in enumerate(pairs):
        res = swap_and_average(pair["q"], pair["a_a"], pair["a_b"])
        results.append(res)
        print(f"  [{i+1}/20] [{pair['q'][:30]}...] Final: {res.final_winner}, Consistent: {res.position_consistent}")
    ...
    # Real judge output for Cohen's Kappa
    print("Running LLM judge on human labeled questions...")
    judge_labels = []
    from openai import OpenAI
    client = OpenAI()
    
    for item in human_data:
        PROMPT = f"""Câu hỏi: {item['question']}\nTrả lời: {item['model_answer']}\nĐánh giá câu trả lời này. Nếu chính xác/đúng trả về 1, sai/thiếu trả về 0. Output JSON duy nhất: {{"label": 0 hoặc 1}}"""
        resp = client.chat.completions.create(
            model=JUDGE_MODEL,
            messages=[
                {"role": "system", "content": "Bạn là expert đánh giá. Chỉ trả lời JSON."},
                {"role": "user", "content": PROMPT}
            ],
            response_format={"type": "json_object"}
        )
        label = json.loads(resp.choices[0].message.content).get("label", 0)
