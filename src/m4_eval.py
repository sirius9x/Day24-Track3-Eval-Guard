...
def evaluate_ragas(questions: list[str], answers: list[str],
    ...
    try:
        ...
        per_question = [EvalResult(
            ...
            faithfulness=float(row.get("faithfulness", 0.0)) if pd.notnull(row.get("faithfulness")) else 0.0,
            answer_relevancy=float(row.get("answer_relevancy", 0.0)) if pd.notnull(row.get("answer_relevancy")) else 0.0,
            context_precision=float(row.get("context_precision", 0.0)) if pd.notnull(row.get("context_precision")) else 0.0,
            context_recall=float(row.get("context_recall", 0.0)) if pd.notnull(row.get("context_recall")) else 0.0)
...
def failure_analysis(eval_results: list[EvalResult], bottom_n: int = 10) -> list[dict]:
    ...
    for res in eval_results:
        ...
        diag, fix = diagnostic_tree.get(worst_metric, ("Unknown", "Unknown"))
